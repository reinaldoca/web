...
import {
  Server, CloudUpload, GitBranch, Blocks, ShieldCheck, Headphones, Router,
  Network, Search, Lightbulb, Users, CheckCircle, Zap
} from 'lucide-react';
...
export async function generateStaticParams() {
  ...
  let params: { lang: Locale; slug: string }[] = [];
  ...
  for (const lang of langs) {
    ...
    const slugs = allServices.map((s: any) => slugify(s.title));
            ...
            <div className="grid md:grid-cols-2 gap-12 lg:gap-16 items-start">
              {(service.toolsAndTechnologies ||
                ...
                <div className="space-y-8 bg-secondary/50 p-8 rounded-lg">
                  ...
                  {service.toolsAndTechnologies && (
                    <div>{renderTools(service.toolsAndTechnologies)}</div>
                  )}

                  {service.migrationStrategies && (
                    <div>
                      <h4 className="font-bold text-xl mb-4">
                        {service.migrationStrategies.title}
                      </h4>
                      <Accordion type="single" collapsible className="w-full">
                        {service.migrationStrategies.strategies.map(
                          (strategy: any) => (
                            <AccordionItem
                              value={strategy.name}
                              key={strategy.name}
                  ...
                  {service.awsPatternsAndServices && (
                    <div>
                      <h4 className="font-bold text-xl mb-4">
                        {service.awsPatternsAndServices.title}
                      </h4>
                      <div className="space-y-4">
                        {Object.entries(
                          service.awsPatternsAndServices.categories
                        ).map(([category, items]) => (
                          <div key={category}>
              ...
              <div className="space-y-8">
                {service.quantifiableBenefits?.length > 0 && (
